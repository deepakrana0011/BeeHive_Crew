...
class TimeSheetScreenProjectDetailsProvider extends BaseProvider{
  ...
  List<ProjectWorkingHourDetail> getTimeForStepper(Checkins detail) {
    ...
    for (int i = 0; i < detail.breaks!.length; i++) {
      if (detail.breaks![i].interval != "Any") {
        var breakStartTimeString = detail.checkInTime!.substring(0, 10) + " " +
            detail.breaks![i].startTime!.replaceAll("PM", "").replaceAll("AM", "");
        var breakEndTimeString = detail.checkInTime!.substring(0, 10) + " " +
            DateFunctions.stringToDateAddMintues(detail.breaks![i].startTime!, int.parse(detail.breaks![i].interval!.substring(0, 2)));
    ...
    if (detail.interuption!.isNotEmpty) {timeString.addAll(detail.interuption!);}
    timeString.sort((a, b) {
